...
增量说明：现在另有可选的 `desktop-directory` 元数据候选驱动，以及只复用已完成群聊捕获的 `backfill-desktop-week`。必须单独校准和授权；原真实 TIM/企微配置不会自动启用。本文下方的微信/KIM实机结果与 TIM/企微未扫描状态保持原口径，参见 [桌面目录候选](DESKTOP_DIRECTORY_CANDIDATE.md)。
