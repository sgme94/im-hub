...
版本：1.0.0rc3。形态：Windows x64 命令行便携包，同时提供 Python wheel 和源码。

## 1. 本次交付的边界

可运行的产品闭环是：配置 → 本地数据库／已完成导出／原生载荷 → 规范化 → 单写者导入 → 只读查询 → 证据汇编 → 校验 → 备份恢复。核心不调用 LLM，没有 Web、Electron 或桌面前端，没有消息发送入口。

本版已完成四平台当前客户端的两轮全自动读取，包括 TIM/企微正常任务栏切换、企微逐条原生复制与三页历史。4,736 条存量消息校验通过，重放新增 0。持续运行与自然新消息观察仍须依据运行记录单独判断。源应用版本只作诊断，实际进程、会话、控件或载荷验证失败才停止。详细配置及 `soak`、停止、恢复方法见[自动采集手册](AUTOMATED_COLLECTION.md)。
...
将 `im-hub-1.0.0rc3-windows-x64.zip` 整体解压，不要只拿走 EXE。目录中的 `_internal` 和 `backend` 是运行依赖；不需要预先安装 Python、Node、npm 或前端，也不会在运行时下载依赖。
...
`tim-ui` / `wecom-ui` 代码已实现有界状态机：实际进程身份 → 桌面租约 → 精确会话 → 导出／选中复制 → 文件／载荷校验 → 来源批次。必须提供已校准的语义控件或图像锚点配置并显式 `--allow-ui`；`soak --activate-clients --until ...` 可在期限内通过系统任务栏自动切换应用。模板不含未经验证的通用坐标。
...
Python 源码安装：`python -m pip install -e .[windows,visual,crypto]`。运行测试：`python -m unittest discover -s tests -v`。构建工具固定在 `.[build]`；便携构建使用 `scripts/build_windows.py`，显式传入已经校验的 Node、ChatLab node_modules、对应源码和许可证。无用户聊天的数据包才允许上传发行页。

四路当前客户端取数和跨应用编排已通过。持续运行、自然新发言的观察差和异常恢复按当前运行段逐项记录；不能把安静时段里的历史重放算作新消息验收。源软件版本不作为拦截项。总体状态按 [RELEASE_GATES.json](RELEASE_GATES.json) 判断，扩展的 24–72 小时测试不是用户本次八小时窗口内已经完成的事实。
