...
时间分为原消息 `event_time/event_ms`、来源 `source_observed_at`、批次导入时间。窗口是 `[since, until)`，必须显式时区。ChatLab 时间为秒，源毫秒精度保留在 sidecar。
