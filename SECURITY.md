...
Calibrated cross-client desktop collection uses a normal click on a unique, semantically verified running-window taskbar button under an explicit authorization deadline. It never changes the foreground lock, attaches input threads, or injects force-focus key sequences. Source version strings remain diagnostic. Pixels locate controls; message data comes only from verified native source formats. Unknown/occluded controls fail closed.

The `soak` command is an explicitly started foreground process, not an installed service or schedule. Each child waits for a fixed parent ownership gate before CLI effects and is placed into a Windows kill-on-close Job Object. A hard absolute end, per-source timeout, STOP file, held-key/input and focus checks bound effects. Child process trees are terminated; the pre-existing IM applications are not. Runtime reports are private and do not contain chat bodies. Repeated transient snapshot/lock errors have a finite retry bound, not unbounded permission or identity retries.
