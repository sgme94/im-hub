...
def _secure_file(path: Path):
    if os.name == 'nt':
        ...
        p = subprocess.run(['icacls.exe', str(path), '/inheritance:r', '/grant:r', principal + ':F', '*S-1-5-18:F'], capture_output=True, timeout=20)
...
def backup(home: Path, output: Path) -> dict:
    ...
    try:
        with ExitStack() as locks:
            ...
            with temp.open('xb'):
                pass
            _secure_file(temp)  # Restrict before any sensitive byte is written.
...
def restore(archive: Path, destination: Path) -> dict:
    ...
    # Short sibling staging avoids adding 40 characters to otherwise-valid Windows
    # paths. The final directory still retains the user's exact requested name.
    stage = destination.with_name('.r-' + uuid.uuid4().hex[:12])
    stage_owned = False
    try:
        with zipfile.ZipFile(archive, 'r') as z:
            infos = z.infolist(); names = [x.filename for x in infos]
            if len(infos) > MAX_ARCHIVE_FILES + 1 or len(set(names)) != len(names) or 'BACKUP.json' not in names:
        ...
        if stage_owned and stage.exists(): shutil.rmtree(stage, ignore_errors=True)
