...
**旧版指定样本群的四路自动取数闭环已通过，不等于全会话覆盖；持续运行与自然新消息观察单列验收。** 当前初始观察尚无自然新发言，不用历史数据重放宣称测得新消息延迟；客户端完整历史和所有附件也不作隐含承诺。源软件版本仅诊断，不设白名单；实际身份/控件/格式问题才停止。进度按[验收清单](docs/RELEASE_GATES.json)与各运行段记录判断。
...
| 企业微信 | 自动单消息原生复制、可见历史分页与回到最新；正常任务栏自动切换 | `wecom-ui` / `wecom-native` / `wecom-json` | 否 |
