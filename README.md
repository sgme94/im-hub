...
开发分支新增 `1.0.0rc3.dev1` TIM 图像锚点/原生控件自动导出：已在指定前景客户端完成真实 4,657 条导出和重复入库核对。**企微自动获取、跨客户端前景编排和自然新消息延迟仍未最终验收**；不是新的完整成品发行。见 [TIM 实机验收](docs/TIM_VISUAL_ACCEPTANCE.md) 和 [本轮验收记录](docs/ACCEPTANCE_DESKTOP_RECONNECT.json)。
...
**微信/KIM已完成当前本地源多轮自动读取；四平台最终验收仍未全部通过**：TIM 指定前景来源的自动导出已实测，企微仍需自动获取/分页，四路均需新到消息验证，长时间无人值守尚未运行。软件版本仅诊断，不设白名单；只有实际能力/身份/格式验证失败才停止。企微当前缺少语义消息控件，不是版本被禁止。见[实时采集说明](docs/LIVE_COLLECTION.md)和[交付门槛](docs/RELEASE_GATES.json)。
